<template>
  ...
  >
    <template slot="body">
      <p class="members-modal__desc" v-if="!addedMember">
        The list of people in your security group won’t be visible at your
        community. Only you can see and manage it.
      </p>
      <p v-else-if="addedMember" class="members-modal__user">
        User
        <span class="members-modal__user-name">{{ addedMember }}</span>
        was successfully added to the security group.
      </p>
      <form class="members-modal__search-field">
        <form-input
          @input="handleInput"
